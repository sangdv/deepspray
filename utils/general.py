...
def get_last_dir(dir, comment=''):
    # Increments a directory runs/exp1 --> runs/exp2_comment
    n = 0  # number
    dir = str(Path(dir))  # os-agnostic
    d = sorted(glob.glob(dir + '*'))  # directories
    if len(d):
        n = max([int(x[len(dir):x.find('_') if '_' in x else None]) for x in d])
    return dir + str(n) + ('_' + comment if comment else '')
...
#     color = color or [random.randint(0, 255) for _ in range(3)]
    ...
    if label == "bag": color = [0, 0, 255] # Đỏ
    elif label == "lobe": color = [255, 102, 0] # Xanh dương
    elif label == "Detached ligament": color = [204, 0, 102] # Tím
    elif label == "drop": color = [0, 153, 255] # Vàng cam
    elif label == "Attached ligament": color = [0, 153, 0] # Xanh lá cây đậm
